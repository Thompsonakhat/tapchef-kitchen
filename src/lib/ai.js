...
async function aiCall(cfg, path, payload = {}, opts = {}) {
  ...
  const timeoutMs = Number(opts.timeoutMs || pickTimeout(cfg));
  const retries = Number.isFinite(Number(opts.retries)) ? Math.max(0, Number(opts.retries)) : 0;
  const url = base + String(path || "");

  for (let attempt = 0; attempt <= retries; attempt += 1) {
    const { ctrl, clear } = withTimeout(timeoutMs);
    try {
      if (DEBUG) console.log("[aiCall] ->", url, payload);
  ...
  return { ok: false, status: 0, json: null, text: "", error: "AI_ERROR" };
}
...
async function routeChat(cfg, userText, systemHint) {
  ...
  try {
    ...
    const { text, json } = await safeRead(r);
    const content = json?.content || json?.message || json?.text || text || "";
...
export async function aiChat(cfg, prompt, { system = "You are a helpful assistant.", meta, retries = 1 } = {}) {
  return aiCall(
    cfg,
    "/chat",
    {
